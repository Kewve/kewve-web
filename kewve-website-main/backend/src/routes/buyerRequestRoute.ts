...
import type { BuyerRequestDeliveryAddress, BuyerRequestDocument } from "../models/BuyerRequest.js";
import { User } from "../models/User.js";
import {
  applyTradeClosureRules,
  computeProducerPayoutPreviewForRequest,
  ensureTrade,
  FULFILLMENT_STATUSES,
  getActiveAllocations,
  invoiceIsPaid,
  isAggregationStyle,
  isLeadOrSoleAllocatedProducer,
  isPrimaryProducer,
  issueTradeInvoiceIfEligible,
  markTradeInvoicePaidFromStripe,
  stripAggregationAllocationsForProducerView,
  stripIssueAdminNotesFromBuyerRequestRow,
...
async function notifyAdminProducerCancelledPaidOrder(
...
function parseRequestDeliveryAddress(
  body: unknown
): { ok: true; value: BuyerRequestDeliveryAddress } | { ok: false; message: string } {
...
const ALLOWED_STATUSES = ["pending", "in_review", "matched", "closed"] as const;

// POST /api/buyer-requests - Buyer creates a request for a product
router.post("/buyer-requests", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const minimumOrderQuantity = Number((product as any).minimumOrderQuantity || 0);
...
router.get("/buyer-requests", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser) {
      res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    if (userHasRole(authUser, "producer")) {
      ...
      const sanitized = (data as Record<string, unknown>[]).map((row) => {
        const safe = stripAggregationAllocationsForProducerView(row as unknown as Record<string, unknown>, uid) as any;
        stripIssueAdminNotesFromBuyerRequestRow(safe);
        const preview = computeProducerPayoutPreviewForRequest(row as any, uid);
...
router.get("/buyer-requests/:id", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser) {
      res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    if (userHasRole(authUser, "producer")) {
      const involved =
        isPrimaryProducer(doc, authUser._id.toString()) ||
        (doc.matchPlan?.allocations || []).some((a: any) => String(a.producerId) === authUser._id.toString());
      if (!involved) {
        res.status(403).json({ success: false, message: "Forbidden." });
        return;
      }
      const payload = stripAggregationAllocationsForProducerView(
      ...
      payload.trade = {
        ...(payload.trade || {}),
        producerPayoutPreview: computeProducerPayoutPreviewForRequest(doc.toObject() as any, authUser._id.toString()),
      };
...
router.post("/buyer-requests/:id/trade/checkout-session", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Trade request ${rid.slice(-8)} · ${inv.platformFeePercent ?? 10}% platform fee on product${
                Number(inv.additionalFeesCents || 0) > 0 ? " · plus add-on fees" : ""
              }`,
            },
...
router.post("/buyer-requests/:id/trade/sync-checkout", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser || !userHasRole(authUser, "buyer")) {
...
router.put("/buyer-requests/:id/status", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser) {
      res.status(401).json({ success: false, message: "Unauthorized" });
    ...
    if (userHasRole(authUser, "producer")) {
      ...
      const isMatchedProducer = (requestDoc.matchPlan?.allocations || []).some(
        (allocation: any) => String(allocation.producerId) === authUser._id.toString()
      );
      ...
      requestDoc.status = nextStatus as any;
      syncLegacyRequestStatusWithTrade(requestDoc);
      await requestDoc.save();
...
router.put("/buyer-requests/:id/trade/producer-decision", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    ...
    const decision = String(req.body?.decision || "").trim() as "accepted" | "declined";
    ...
    const aggLines = planAllocs.filter((a: any) => String(a.producerId) === uid && Number(a.allocatedKg || 0) > 0);
    const activeCount = planAllocs.filter((a: any) => Number(a.allocatedKg || 0) > 0).length;
    const usePerProducerResponses =
      planAllocs.length > 0 && (requestDoc.fulfillmentMode === "aggregation" || activeCount > 1);
    ...
    if (usePerProducerResponses) {
      ...
      const productIdFilter = String((req.body as any)?.productId || "").trim();
      const candidates = productIdFilter
        ? aggLines.filter((a: any) => String(a.productId) === productIdFilter)
        : aggLines;
      ...
      const line = candidates[0] as any;
...
router.post("/buyer-requests/:id/trade/invoice", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser || !userHasRole(authUser, "producer")) {
...
router.put("/buyer-requests/:id/trade/invoice/mark-paid", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser || !userHasRole(authUser, "producer")) {
      res.status(403).json({ success: false, message: "Forbidden." });
...
router.put("/buyer-requests/:id/trade/fulfillment", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser || !userHasRole(authUser, "producer")) {
    ...
    const myAcceptedAllocs = activeAllocs.filter(
      (a: any) => String(a.producerId) === uid && String(a.producerResponse || "pending") === "accepted"
    );
    ...
    if (isAgg) {
      ...
      if (productIdRaw) {
        target = myAcceptedAllocs.filter((a: any) => String(a.productId) === productIdRaw);
        if (!target.length) {
      ...
      for (const line of target) {
        line.fulfillmentStatus = next as any;
      }
...
router.put("/buyer-requests/:id/trade/buyer-receipt", authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const authUser = req.user;
    if (!authUser || !userHasRole(authUser, "buyer")) {
