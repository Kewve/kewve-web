...
function productToClientJson(product: { toObject?: () => any } | any) {
...
const validateProductPayload = (payload: any): string | null => {
...
router.get("/products", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const isBuyer = shouldUseBuyerProductCatalog(req.user, req.query as Record<string, unknown>);
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const upload = multer({
        ...
        fileFilter: (_req: any, file: Express.Multer.File, cb: any) => {
          if (ALLOWED_COMPLIANCE_TYPES.includes(file.mimetype)) cb(null, true);
      ...
      fieldsUpload(req, res, async (err) => {
        ...
        const filesMap = ((req as any).files || {}) as Record<string, Express.Multer.File[]>;
        const files = [
...
router.post("/products/:id/compliance-documents", authenticate, async (req: AuthRequest, res: Response) => {
  try {
    if (!userHasRole(req.user, "producer")) {
...
router.get(
  "/products/:id/compliance-documents/:docId/file",
  tokenFromQuery,
  authenticate,
...
router.delete(
  "/products/:id/compliance-documents/:docId",
  authenticate,
  async (req: AuthRequest, res: Response) => {
    try {
      if (!userHasRole(req.user, "producer")) {
      ...
      (product as any).complianceDocuments = (product.complianceDocuments || []).filter(
        (d: any) => d._id?.toString() !== req.params.docId
      );
      ...
      if ((product.complianceDocuments || []).length === 0) {
        (product as any).verification = "pending";
        (product as any).readiness = "draft";
