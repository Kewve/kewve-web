...
import { BuyerRequest, type BuyerRequestDocument } from "../models/BuyerRequest.js";
import { AggregationCluster } from "../models/AggregationCluster.js";
import {
  applyTradeClosureRules,
  ensureTrade,
  FULFILLMENT_STATUSES,
  getActiveAllocations,
  invoiceProducerPayoutCents,
  isAggregationStyle,
  invoiceIsPaid,
  issueTradeInvoiceIfEligible,
  syncLegacyRequestStatusWithTrade,
  syncTradeFulfillmentFromAllocations,
  syncTradeProducerDecisionFromAllocations,
  tradeIsTerminalClosed,
  verifyPayoutPasscode,
} from "../utils/buyerRequestTrade.js";
import { notifyBuyerInvoiceSent, notifyProducersTradePaid } from "../utils/tradeNotifyMail.js";
import { transferToConnectAccount } from "../services/stripePayout.js";
import {
  stripComplianceDocumentsForClient,
  syncProductComplianceStatus,
} from "../utils/productCompliance.js";

const router = Router();

function adminProductJson(product: Record<string, unknown>): Record<string, unknown> {
  const o = { ...product };
  if (Array.isArray(o.complianceDocuments) && o.complianceDocuments.length) {
    o.complianceDocuments = stripComplianceDocumentsForClient(o.complianceDocuments as any[]);
  }
  return o;
}

function tradeIssueNoteAuthor(req: AuthRequest): { authorId?: Types.ObjectId; authorName: string } {
  const u = req.user as any;
  if (u?._id) {
    return { authorId: u._id as Types.ObjectId, authorName: String(u.name || u.email || "Admin") };
  }
  return { authorName: String(u?.name || u?.email || "Admin") };
}

function pushTradeIssueAdminNote(t: NonNullable<BuyerRequestDocument["trade"]>, req: AuthRequest, body: string): void {
...
async function stripeRefundPaidTradeInvoice(
...
type AssessmentDropoffRange = "7d" | "30d" | "90d";

const ASSESSMENT_RANGE_DAYS: Record<AssessmentDropoffRange, number> = {
  "7d": 7,
  "30d": 30,
  "90d": 90,
};

function toAssessmentRange(input: unknown): AssessmentDropoffRange {
  const v = String(input || "").trim().toLowerCase();
  if (v === "7d" || v === "30d" || v === "90d") return v;
  return "30d";
}

function assessmentConversionRate(count: number, previous: number | null): number | null {
...
router.get("/admin/analytics/assessment-dropoff", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const assessments = await Assessment.find({ createdAt: { $gte: from, $lte: now } })
      .select(
        "createdAt country businessRegistered productRegulatoryApproval traceToCustomers haccpCertification monthlyProductionCapacity exportGradeCartons labelsInEnglish shelfLifeInfo documents deliverToUK exportPricing confirmAccuracy agreeCompliance"
      )
      .lean();

    const stages = [
      { key: "started", label: "Started assessment", done: (a: any) => hasTruthy(a.createdAt) },
      { key: "export_context", label: "Export context", done: (a: any) => hasTruthy(a.country) },
      { key: "business_legal", label: "Business & legal readiness", done: (a: any) => hasTruthy(a.businessRegistered) },
      { key: "product_definition", label: "Product definition", done: (a: any) => hasTruthy(a.productRegulatoryApproval) },
      { key: "traceability", label: "Product traceability", done: (a: any) => hasTruthy(a.traceToCustomers) },
      { key: "food_safety", label: "Food safety", done: (a: any) => hasTruthy(a.haccpCertification) },
      { key: "capacity", label: "Production capacity", done: (a: any) => hasTruthy(a.monthlyProductionCapacity) },
      { key: "packaging", label: "Packaging readiness", done: (a: any) => hasTruthy(a.exportGradeCartons) },
      { key: "labelling", label: "Labelling compliance", done: (a: any) => hasTruthy(a.labelsInEnglish) },
      { key: "shelf_life", label: "Shelf life", done: (a: any) => hasTruthy(a.shelfLifeInfo) },
      { key: "documentation", label: "Documentation readiness", done: (a: any) => Array.isArray(a.documents) && a.documents.length > 0 },
      { key: "logistics", label: "Logistics understanding", done: (a: any) => hasTruthy(a.deliverToUK) },
      { key: "financial", label: "Financial readiness", done: (a: any) => hasTruthy(a.exportPricing) },
      {
        key: "compliance_confirmation",
        label: "Compliance confirmation",
        done: (a: any) => String(a.confirmAccuracy || "") === "yes" && String(a.agreeCompliance || "") === "yes",
      },
    ];
    ...
    const stageCompletionCounts = new Array<number>(stages.length).fill(0);
...
function escapeAdminSearchRegex(s: string): string {
  return s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
}

/** Global search for ops: products, buyer requests, clusters, producers (admin JWT). */
router.get("/admin/search", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const refSuffixFromId = (id: string): string => {
      const hex = id.replace(/[^a-f0-9]/gi, "");
      return hex.length >= 4 ? hex.slice(-4) : hex;
    };

    const products: Array<{ _id: string; name?: string; category?: string }> = [];
    const buyerRequests: Array<{
      _id: string;
      refSuffix: string;
      productName?: string;
      buyerName?: string;
    ...
    const clusters: Array<{ _id: string; productName?: string; clusterId?: string; status?: string }> = [];
    const producers: Array<{ _id: string; name?: string; email?: string }> = [];
...
router.get("/admin/buyer-requests", authenticateAdmin, async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.put("/admin/buyer-requests/:id/status", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.post("/admin/buyer-requests/:id/auto-match", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    let candidates: any[] = await Product.find({
      ...baseFilter,
    ...
    const bestByProducer = new Map<string, any>();
    ...
    const shortlist = Array.from(bestByProducer.values()).sort((a: any, b: any) => {
    ...
    const producerIds = shortlist.map((item: any) => item.userId);
    const producers = await User.find({ _id: { $in: producerIds } }).select("name").lean();
    const producerNameMap = new Map(producers.map((p: any) => [String(p._id), p.name || "Producer"]));
...
router.put("/admin/buyer-requests/:id/match-plan", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const requestDoc = await BuyerRequest.findById(req.params.id);
    if (!requestDoc) {
      res.status(404).json({ success: false, message: "Buyer request not found." });
    ...
    const candidateProductIds = incomingAllocations
      .map((a: any) => String(a?.productId || "").trim())
      .filter(Boolean);
    ...
    const producerIds = Array.from(new Set(products.map((p: any) => String(p.userId))));
    const producers = await User.find({ _id: { $in: producerIds } }).select("name").lean();
    const producerNameMap = new Map(producers.map((p: any) => [String(p._id), p.name || "Producer"]));

    const prevAllocs = (requestDoc.matchPlan?.allocations || []) as any[];
    ...
    for (const allocation of incomingAllocations) {
      ...
      if (!productId || !producerId) continue;
      if (!Number.isFinite(allocatedKg) || allocatedKg <= 0) continue;

      const product = productMap.get(productId);
      if (!product) continue;
      if (String(product.userId) !== producerId) continue;
    ...
    for (const s of sanitizedAllocations) {
      const prev = prevAllocs.find(
        (p: any) => String(p.producerId) === String(s.producerId) && String(p.productId) === String(s.productId)
      );
    ...
    requestDoc.matchPlan = {
      ...
      allocations: sanitizedAllocations as any,
    } as any;
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (incomingAllocations.length > 0) {
        ...
        if (first) {
          requestDoc.producerId = first.producerId as any;
          requestDoc.productId = first.productId as any;
      ...
      const producerName =
        (await User.findById(product.userId).select("name").lean())?.name || "Producer";
      ...
      const allocations = [
        {
          ...
          availableCapacityKg,
          unitPrice: Number(product.unitPrice || 0),
          producerResponse: "pending" as const,
          fulfillmentStatus: "none" as const,
          declinedReason: "",
        },
      ];

      requestDoc.producerId = product.userId as any;
      requestDoc.productId = product._id as any;
...
router.post("/admin/buyer-requests/:id/trade/refund", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const requestDoc = await BuyerRequest.findById(req.params.id);
    if (!requestDoc) {
      res.status(404).json({ success: false, message: "Buyer request not found." });
...
router.post("/admin/buyer-requests/:id/trade/invoice", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const requestDoc = await BuyerRequest.findById(req.params.id);
    if (!requestDoc) {
      res.status(404).json({ success: false, message: "Buyer request not found." });
    ...
    const rawExtra = (req.body as Record<string, unknown>)?.additionalFeesCents;
    ...
    const additionalFeesNote = String((req.body as Record<string, unknown>)?.additionalFeesNote ?? "")
      .trim()
      .slice(0, 200);
...
router.put("/admin/buyer-requests/:id/trade/invoice/mark-paid", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const requestDoc = await BuyerRequest.findById(req.params.id);
    if (!requestDoc) {
      res.status(404).json({ success: false, message: "Buyer request not found." });
...
router.put("/admin/buyer-requests/:id/trade/fulfillment", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const next = String(req.body?.status || "").trim();
    if (!FULFILLMENT_STATUSES.includes(next as (typeof FULFILLMENT_STATUSES)[number])) {
    ...
    if (isAggregationStyle(requestDoc)) {
      ...
      const accepted = lines.filter((a: any) => String(a.producerResponse || "pending") === "accepted");
      const target = accepted.length ? accepted : lines;
      for (const line of target) {
        (line as any).fulfillmentStatus = next as any;
      }
      syncTradeFulfillmentFromAllocations(requestDoc);
    } else {
      t.fulfillmentStatus = next as any;
    }
    applyTradeClosureRules(requestDoc);
    await requestDoc.save();
    res.status(200).json({ success: true, data: requestDoc });
...
router.post("/admin/buyer-requests/:id/trade/issue-note", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
...
router.put("/admin/buyer-requests/:id/trade/resolve-issues", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const requestDoc = await BuyerRequest.findById(req.params.id);
    if (!requestDoc) {
      res.status(404).json({ success: false, message: "Buyer request not found." });
    ...
    if (refundBuyer) {
      const noteForRefund =
        resolutionNote || String(req.body?.refundNote || "").trim().slice(0, 1000) || "Issue resolution — refund to buyer";
      const refundRes = await stripeRefundPaidTradeInvoice(
        requestDoc,
        refundAmount,
        noteForRefund,
        "admin_issue_resolution_refund"
      );
      if (!refundRes.ok) {
...
router.get("/admin/accounts/payout-queue", authenticateAdmin, async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const rows = await BuyerRequest.find({
    ...
    for (const r of rows as any[]) {
      ...
      if (isAggregationStyle(r as any)) {
        const active = (r.matchPlan?.allocations || []).filter((a: any) => Number(a?.allocatedKg || 0) > 0);
        const accepted = active.filter((a: any) => String(a.producerResponse || "pending") === "accepted");
        const acceptedGrosses = accepted.map((a: any) => Math.round(Number(a.unitPrice || 0) * Number(a.allocatedKg || 0) * 100));
        const acceptedTotal = acceptedGrosses.reduce((sum: number, g: number) => sum + g, 0);
        ...
        for (const a of accepted) {
          ...
          if (!["none", "failed"].includes(ps)) continue;
          if (String(a?.fulfillmentStatus || "none") !== "completed") continue;
          queue.push({
            ...r,
            payoutTarget: {
              mode: "allocation",
...
router.get("/admin/accounts/payout-history", authenticateAdmin, async (_req: AuthRequest, res: Response): Promise<void> => {
  try {
    const rows = await BuyerRequest.find({
      "trade.invoice.paidAt": { $exists: true, $ne: null },
      "trade.producerDecision": "accepted",
    ...
    for (const r of rows as any[]) {
      ...
      if (isAggregationStyle(r as any)) {
        ...
        for (const a of accepted) {
          const idx = accepted.indexOf(a);
          const ps = String(a?.payout?.status || "none");
          if (!["pending", "completed", "failed"].includes(ps)) continue;
          const gross = acceptedGrosses[idx] || 0;
          history.push({
            ...r,
            payoutTarget: {
...
router.post("/admin/accounts/payout", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const amountOverride =
      req.body?.amountCents !== undefined && req.body?.amountCents !== null && req.body?.amountCents !== ""
    ...
    if (isAggregationStyle(requestDoc)) {
      ...
      const active = (requestDoc.matchPlan?.allocations || []).filter((a: any) => Number(a?.allocatedKg || 0) > 0);
      const accepted = active.filter((a: any) => String(a.producerResponse || "pending") === "accepted");
      const line = accepted.find((a: any) => String(a.producerId) === producerId && String(a.productId) === productId);
      if (!line) {
        res.status(404).json({ success: false, message: "Allocation line not found for payout." });
        return;
      }
      if (String((line as any).fulfillmentStatus || "none") !== "completed") {
        res.status(400).json({ success: false, message: "Producer line must be completed before payout." });
        return;
      }
      const ps = String((line as any).payout?.status || "none");
      if (ps === "completed") {
      ...
      const acceptedGrosses = accepted.map((a: any) => Math.round(Number(a.unitPrice || 0) * Number(a.allocatedKg || 0) * 100));
      const lineIdx = accepted.findIndex((a: any) => String(a.producerId) === producerId && String(a.productId) === productId);
      ...
      (line as any).payout = (line as any).payout || { status: "none" };
      (line as any).payout.status = "pending";
      (line as any).payout.amountCents = payoutCents;
      (line as any).payout.initiatedAt = new Date();
      await requestDoc.save();
      ...
      try {
        ...
        (line as any).payout.status = "completed";
        (line as any).payout.stripeTransferId = transfer.id;
        (line as any).payout.errorMessage = undefined;
        await requestDoc.save();
        res.status(200).json({ success: true, data: requestDoc, transferId: transfer.id });
      } catch (err: any) {
        (line as any).payout.status = "failed";
        (line as any).payout.errorMessage = err?.message || "Stripe transfer failed";
        await requestDoc.save();
        res.status(502).json({ success: false, message: (line as any).payout.errorMessage, data: requestDoc });
      }
      return;
    }
...
router.put("/admin/producer/:id/document/:docId/review", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (action === "rejected") {
      ...
      (doc as any).rejectionReason = undefined;
      (doc as any).reviewedAt = undefined;

      // If the producer was verified based on documents, undo that too.
      if ((assessment as any).verified) {
        (assessment as any).verified = false;
        (assessment as any).verifiedAt = undefined;
        (assessment as any).verifiedBy = undefined;
      }
    }
...
router.get("/admin/products/:id", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const payload = adminProductJson({ ...(product as any) });
    res.status(200).json({
      success: true,
      data: {
        ...payload,
...
router.get("/admin/products/:id/compliance-documents/:docId/file", async (req: AuthRequest, res: Response, next: Function): Promise<void> => {
  if (!req.headers.authorization && req.query.token) {
    req.headers.authorization = `Bearer ${req.query.token}`;
  }
  authenticateAdmin(req, res, next);
  ...
  try {
    ...
    const doc = (product.complianceDocuments || []).find(
...
router.put(
  "/admin/products/:id/compliance-documents/:docId/review",
  authenticateAdmin,
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      const { action, reason } = req.body;
      ...
      syncProductComplianceStatus(product as any);
      await product.save();

      res.status(200).json({
        success: true,
        message: `Document ${action}`,
        data: adminProductJson(product.toObject() as any),
      });
    } catch (error) {
      ...
      if (docs.length >= 1) {
        ...
        if (!allApproved) {
          res.status(400).json({
            success: false,
            message:
              "This product has compliance documents. Approve each document individually. The listing goes live only when every document is approved.",
          });
          return;
        }
      }
...
router.delete("/admin/products/:id", authenticateAdmin, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await AggregationCluster.updateMany(
      { "contributions.productId": product._id },
      { $pull: { contributions: { productId: product._id } } }
    );
