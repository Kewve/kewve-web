import type { SavedDeliveryAddress, UserDocument } from "../models/User.js";

const STRIPE_ACCT_ID = /^acct_[a-zA-Z0-9]+$/;

export function isValidStripeConnectAccountId(value: string): boolean {
  const t = String(value || "").trim();
  if (!t) return true;
  return STRIPE_ACCT_ID.test(t);
}

export type AppRole = "producer" | "buyer" | "admin";

export function normalizeRoles(user: { roles?: string[]; role?: string } | null | undefined): AppRole[] {
  if (!user) return [];
  const raw = user.roles?.filter((r): r is AppRole =>
    ["producer", "buyer", "admin"].includes(String(r).toLowerCase())
  );
  if (raw && raw.length > 0) return Array.from(new Set(raw.map((r) => r.toLowerCase() as AppRole)));
  const r = String(user.role || "producer").toLowerCase() as AppRole;
  return [r];
}
...
export function shouldUseBuyerProductCatalog(
  user: { roles?: string[]; role?: string } | null | undefined,
  query: Record<string, unknown>
): boolean {
  if (!userHasRole(user, "buyer")) return false;
  const catalog = String(query.catalog || query.forBuyer || "").toLowerCase();
  if (catalog === "buyer" || catalog === "true" || catalog === "1") return true;
  return !userHasRole(user, "producer");
}

export function userPublicJson(user: UserDocument) {
  ...
  let buyerDelivery: SavedDeliveryAddress | undefined;
