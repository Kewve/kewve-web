...
import type {
  AggregationClusterDocument,
  ClusterContribution,
  ClusterSettlement,
  ClusterSettlementEntry,
} from "../models/AggregationCluster.js";
import { transferToConnectAccount } from "../services/stripePayout.js";

const DEFAULT_PLATFORM_FEE_PERCENT = 10;

export function computeInvoiceCentsFromSubtotal(subtotalCents: number, platformFeePercent = DEFAULT_PLATFORM_FEE_PERCENT) {
...
export function allocateVolumeAcrossContributions(
  contributions: ClusterContribution[],
  volumeKg: number
): { allocations: VolumeAllocation[]; remainingKg: number; totalAllocatedKg: number } {
...
export async function buildSettlementForCluster(
  cluster: AggregationClusterDocument,
  volumeKg: number,
  market: string,
  timeline: string,
  platformFeePercent = DEFAULT_PLATFORM_FEE_PERCENT,
  additionalFeesCents = 0
): Promise<{ settlement?: ClusterSettlement; error?: string }> {
  const { allocations, remainingKg, totalAllocatedKg } = allocateVolumeAcrossContributions(
  ...
  for (const p of products) {
    priceById.set(String(p._id), Number((p as any).unitPrice || 0));
  }
  ...
  const entries: ClusterSettlementEntry[] = allocations.map((row, i) => {
    ...
    return {
      contributionId: row.contribution._id as Types.ObjectId,
      producerId: row.contribution.producerId as Types.ObjectId,
      producerName: row.contribution.producerName,
      productId: row.contribution.productId as Types.ObjectId,
      ...
      adjustmentCents,
      netPayoutCents,
      supplyStatus: "pending" as const,
      payout: { status: "none" as const },
    };
  });

  const settlement: ClusterSettlement = {
    computedAt: new Date(),
...
export async function applyClusterPurchaseFromPaidSession(params: {
  ...
  if (!doc) return { applied: false, skipReason: "cluster_not_found" };

  if ((doc as any).purchase?.paidAt) {
  ...
  const prevPurchase: any = doc.purchase || {};
  const now = new Date();
  doc.purchase = {
    buyerId: prevPurchase.buyerId || (new Types.ObjectId(buyerId) as any),
  ...
  doc.settlement = settlement as ClusterSettlement;
...
export async function tryPayoutClusterSettlementEntry(
  clusterId: string,
  entryId: string
  ...
  const entry = entries.find((e: any) => String(e._id) === entryId);
  if (!entry) return { ok: false, message: "Settlement entry not found." };
  ...
  const payout = entry.payout || { status: "none" as const };
