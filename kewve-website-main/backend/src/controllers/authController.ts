...
import { User, UserDocument, type SavedDeliveryAddress } from "../models/User.js";
  ...
  try {
    ...
    if (existing) {
      ...
      res.status(409).json({
        success: false,
        message:
          "An account with this email already exists. Log in as a buyer and use “Producer dashboard” in the buyer portal to add producer access (payment required).",
      });
      return;
    }
  ...
  try {
    ...
    if (expectedRole === "buyer" && !userHasRole(user, "buyer")) {
      res.status(403).json({
        success: false,
        message:
          "This account does not have buyer access. Log in from the main login page and enable buyer access from your producer dashboard, or register as a buyer with a different email.",
      });
      return;
    }
    ...
    if (expectedRole === "producer" && !userHasRole(user, "producer")) {
      res.status(403).json({
        success: false,
        message:
          "This account does not have producer access. Use the buyer dashboard to complete producer signup (payment), or register as a producer.",
      });
      return;
    }

    const buyerOnly =
      userHasRole(user, "buyer") && !userHasRole(user, "producer") && !userHasRole(user, "admin");
...
export const getCurrentUser = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as { user?: UserDocument };
    ...
    if (Object.prototype.hasOwnProperty.call(body, "savedDeliveryAddress")) {
      ...
      if (raw === null) {
        ...
        user.savedDeliveryAddress = {
          ...
          phone,
          company,
        } satisfies SavedDeliveryAddress;
        updated = true;
      }
    }
...
export const enableBuyerRole = async (req: Request, res: Response): Promise<void> => {
  try {
    const authReq = req as { user?: UserDocument };
    const user = authReq.user;
...
export const upgradeBuyerFromPayment = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { buyerId, customerEmail, discountCode } = req.body as {
      buyerId?: string;
