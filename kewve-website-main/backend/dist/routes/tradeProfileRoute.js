...
router.get("/trade-profile", authenticate, async (req, res) => {
    try {
...
router.put("/trade-profile", authenticate, async (req, res) => {
    try {
        const userId = req.user._id;
        const { companyName, country, description, yearsOfExperience, marketsPreviouslyExportedTo, monthlyProductionCapacity, processingMethods, packagingFormats, storageFacilities, sustainabilityPractices, traceabilitySystems, completedSections, } = req.body;
        const profile = await TradeProfile.findOneAndUpdate({ userId }, {
            userId,
            companyName,
            country,
            description,
