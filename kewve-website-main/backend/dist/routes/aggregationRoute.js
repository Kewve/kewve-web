...
import { applyClusterPurchaseFromPaidSession, buildSettlementForCluster, tryPayoutClusterSettlementEntry, } from "../utils/clusterSettlement.js";
...
const router = Router();
async function notifyAdminClusterEvent(subject, lines) {
    try {
...
const recalcClusterStatus = (cluster) => {
    const approvedContributions = (cluster.contributions || []).filter((c) => c.status === "approved");
    const totalApprovedVolumeKg = approvedContributions.reduce((sum, c) => sum + Number(c.committedKg || 0), 0);
...
const clusterInvoiceIssued = (cluster) => !!cluster?.purchase?.invoice?.sentAt;
const clusterLockedForSupplyChanges = (cluster) => !!cluster?.purchase?.paidAt || clusterInvoiceIssued(cluster);
const generateClusterId = async () => {
...
// Admin: list clusters
router.get("/admin/clusters", authenticateAdmin, async (_req, res) => {
    try {
...
router.post("/admin/clusters", authenticateAdmin, async (req, res) => {
    try {
        const { clusterId, productName, category, productForm, targetMarket, supplyCountry, minimumExportVolumeKg, availabilityWindow, specificationSummary, } = req.body || {};
        ...
        const cluster = await AggregationCluster.create({
            ...
            createdBy: (req.user?.email || "admin").toString(),
        });
...
router.delete("/admin/clusters/:id", authenticateAdmin, async (req, res) => {
    try {
...
router.put("/admin/clusters/:id/status", authenticateAdmin, async (req, res) => {
    try {
...
router.put("/admin/clusters/:id/contributions/:contributionId", authenticateAdmin, async (req, res) => {
    try {
        ...
        const contribution = (cluster.contributions || []).find((c) => String(c._id) === req.params.contributionId);
        if (!contribution) {
...
router.post("/admin/clusters/:id/contributions", authenticateAdmin, async (req, res) => {
    try {
        const { producerEmail, producerId, productId, productName, committedKg, status } = (req.body || {});
        ...
        if (productIdStr) {
            ...
            if (String(prod.readiness || "") !== "approved" || String(prod.verification || "") !== "verified") {
                res.status(400).json({ success: false, message: "Product must be approved and verified to join a cluster." });
                return;
            }
            const user = await User.findById(prod.userId).lean();
            if (!user) {
            ...
            if (String(prod.category || "").toLowerCase() !== String(cluster.category || "").toLowerCase()) {
            ...
            if (supply && !countriesMatch(user.country, supply)) {
                res.status(400).json({
                    success: false,
            ...
            const maxKg = Number(prod.monthlyCapacity || 0);
            ...
            const already = (cluster.contributions || []).some((c) => String(c.producerId) === String(user._id));
            if (already) {
            ...
            cluster.contributions.push({
                producerId: user._id,
                producerName: String(user.businessName || user.name || user.email || "Producer"),
                productId: prod._id,
                productName: String(prod.name || "Product"),
...
router.delete("/admin/clusters/:id/contributions/:contributionId", authenticateAdmin, async (req, res) => {
    try {
        const cluster = await AggregationCluster.findById(req.params.id);
        if (!cluster) {
            res.status(404).json({ success: false, message: "Cluster not found." });
        ...
        cluster.contributions = (cluster.contributions || []).filter((c) => String(c._id) !== String(req.params.contributionId));
...
const SUPPLY_STATUSES = ["pending", "delivered", "verified", "accepted"];
function parseClusterCustomDelivery(body) {
    const b = body;
    const d = b?.address;
...
router.put("/clusters/:clusterId/settlement/entries/:entryId/supply", authenticate, async (req, res) => {
    try {
        ...
        const entry = cluster.settlement.entries.find((e) => String(e._id) === req.params.entryId);
        if (!entry) {
...
router.put("/admin/clusters/:clusterId/settlement/entries/:entryId/supply", authenticateAdmin, async (req, res) => {
    try {
        const supplyStatus = String(req.body?.supplyStatus || "").trim();
        if (!SUPPLY_STATUSES.includes(supplyStatus)) {
        ...
        entry.supplyStatus = supplyStatus;
        cluster.markModified("settlement");
        cluster.markModified("settlement.entries");
        await cluster.save();
...
router.post("/admin/clusters/:clusterId/settlement/entries/:entryId/payout", authenticateAdmin, async (req, res) => {
    try {
...
router.put("/admin/clusters/:clusterId/settlement/entries/:entryId/payout-amount", authenticateAdmin, async (req, res) => {
    try {
        ...
        const otherEntriesTotal = (cluster.settlement.entries || []).reduce((sum, row) => {
...
router.get("/clusters/eligible", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
        ...
        const data = clusters.map((cluster) => {
            const categoryMatch = producerProducts.filter((p) => String(p.category || "").toLowerCase() === String(cluster.category || "").toLowerCase());
            const ownContribution = (cluster.contributions || []).find((c) => String(c.producerId) === String(authUser._id));
            ...
            else if (alreadyJoined && !canRetry)
                reason = ownContribution?.status === "pending" ? "Awaiting admin review" : "Already approved in cluster";
            else if (canRetry)
                reason = ownContribution?.notes ? `Rejected: ${ownContribution.notes}` : "Rejected. You can retry.";
            return {
                ...cluster,
...
router.post("/clusters/:id/join", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            res.status(401).json({ success: false, message: "Unauthorized" });
        ...
        const existingContribution = (cluster.contributions || []).find((c) => String(c.producerId) === String(authUser._id));
        ...
        if (existingContribution && existingContribution.status === "rejected") {
            ...
            existingContribution.productId = product._id;
...
router.get("/buyer/clusters", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser) {
            res.status(401).json({ success: false, message: "Unauthorized" });
        ...
        const requestableClusters = clusters.filter((cluster) => {
            ...
            return (!paid &&
                (!sent || ownedDraft) &&
                (cluster.contributions || []).some((c) => c.status === "approved" && Number(c.committedKg || 0) > 0));
        });
...
router.get("/buyer/clusters/my-purchases", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
...
router.get("/buyer/clusters/:id/quote", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser || !userHasRole(authUser, "buyer")) {
        ...
        const { settlement, error } = await buildSettlementForCluster(cluster, volumeKg, market, timeline, undefined, additionalFeesCents);
        if (error || !settlement) {
...
router.post("/buyer/clusters/:id/checkout-session", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser || !userHasRole(authUser, "buyer")) {
...
router.post("/buyer/clusters/:id/sync-checkout", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser || !userHasRole(authUser, "buyer")) {
...
router.put("/buyer/clusters/:id/receipt", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser || !userHasRole(authUser, "buyer")) {
...
router.put("/admin/clusters/:id/resolve-issues", authenticateAdmin, async (req, res) => {
    try {
        const cluster = await AggregationCluster.findById(req.params.id);
        ...
        if (closeOk) {
            cluster.purchase.buyerReceipt = "received_ok";
            cluster.purchase.buyerReceiptAt = new Date();
...
router.post("/admin/clusters/:id/refund", authenticateAdmin, async (req, res) => {
    try {
        const cluster = await AggregationCluster.findById(req.params.id);
        if (!cluster || !cluster.purchase?.paidAt) {
            res.status(404).json({ success: false, message: "Paid cluster not found." });
        ...
        const purchaseRefund = cluster.purchase.refund || { status: "none" };
        ...
        cluster.purchase.refund = purchaseRefund;
        cluster.markModified("purchase");
        await cluster.save();
        try {
...
router.put("/admin/clusters/:id/delivery", authenticateAdmin, async (req, res) => {
    try {
        ...
        if (mode === "buyer_profile") {
            ...
            if (!line1 || !city || !postalCode || !country) {
                res.status(400).json({
                    success: false,
                    message: "Buyer has no complete saved delivery address. Ask them to fill it under their profile, or use custom mode.",
                });
                return;
            }
...
router.get("/clusters/my-settlements", authenticate, async (req, res) => {
    try {
        const authUser = req.user;
        if (!authUser || !userHasRole(authUser, "producer")) {
        ...
        const data = clusters.map((c) => ({
            ...
            settlement: c.settlement
                ? {
                    ...c.settlement,
                    entries: (c.settlement.entries || []).filter((e) => String(e.producerId) === String(authUser._id)),
                }
                : undefined,
        }));
        res.status(200).json({ success: true, data });
