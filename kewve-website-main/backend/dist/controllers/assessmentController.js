...
export const createOrUpdateAssessment = async (req, res) => {
    try {
        ...
        const assessment = await Assessment.findOneAndUpdate({ userId }, {
            ...assessmentData,
            userId,
        }, {
            new: true,
...
export const getAssessment = async (req, res) => {
    try {
        ...
        if (assessmentObj.documents) {
            assessmentObj.documents = assessmentObj.documents.map((doc) => ({
                _id: doc._id,
                name: doc.name,
                type: doc.type,
                size: doc.size || 0,
...
export const updateChecklistState = async (req, res) => {
    try {
        const userId = req.user._id; // Keep as ObjectId, not string
        ...
        const assessment = await Assessment.findOneAndUpdate({ userId }, { checklistState }, { new: true, upsert: true });
        res.status(200).json({
            success: true,
