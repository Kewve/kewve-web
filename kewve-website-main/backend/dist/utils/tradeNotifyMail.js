...
function producerUserIdsToNotify(doc) {
    const ids = new Set();
    ids.add(String(doc.producerId));
    const allocs = doc.matchPlan?.allocations || [];
    const aggregation = doc.fulfillmentMode === "aggregation" || allocs.filter((a) => Number(a?.allocatedKg || 0) > 0).length > 1;
    if (aggregation) {
        for (const a of allocs) {
            if (Number(a?.allocatedKg || 0) <= 0)
                continue;
...
export async function notifyProducersTradePaid(doc) {
    const inv = doc.trade?.invoice;
    if (!inv?.paidAt || !inv.totalCents)
        return;
    ...
    for (const u of users) {
        ...
        if (!email || seen.has(email))
            continue;
        seen.add(email);
        targets.push({ email, name: String(u.name || "Producer") });
    }
    if (!targets.length) {
