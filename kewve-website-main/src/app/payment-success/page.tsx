...
import { completeBuyerProducerUpgrade } from '@/actions/buyerProducerUpgrade';
...
function PaymentSuccessContent() {
  ...
  useEffect(() => {
    ...
    const handleSuccess = async () => {
      const isBuyerUpgrade = flow === 'buyer_upgrade';
      const result = isBuyerUpgrade
        ? await completeBuyerProducerUpgrade(sessionId)
        ...
        <div className='bg-white rounded-lg shadow-lg p-8 text-center'>
          {status === 'loading' && (
            <>
              ...
              <h1 className={`text-2xl font-bold text-gray-900 mb-2 ${titleFont.className}`}>
                {isBuyerUpgrade ? 'Activating producer access' : 'Setting Up Your Account'}
              </h1>
              <p className={`text-sm text-gray-500 ${josefinRegular.className}`}>
                {isBuyerUpgrade
                  ? 'Payment verified. Upgrading your account...'
                  : 'Payment verified. Creating your account...'}
              </p>
              <p className={`text-xs text-gray-500 mt-3 ${josefinRegular.className}`}>{GDPR.paymentsCheckout}</p>
            </>
          )}
          ...
          {status === 'success' && (
            <>
              ...
              <h1 className={`text-2xl font-bold text-gray-900 mb-2 ${titleFont.className}`}>
                {isBuyerUpgrade ? 'Producer dashboard ready' : 'Welcome to Kewve!'}
              </h1>
              <p className={`text-sm text-gray-500 mb-4 ${josefinRegular.className}`}>
                {isBuyerUpgrade
                  ? 'Your payment is confirmed and your producer account is active. Redirecting...'
                  : 'Your account has been created and payment confirmed. Redirecting to your dashboard...'}
              </p>
              <p className={`text-xs text-gray-500 mb-4 ${josefinRegular.className}`}>{GDPR.paymentsCheckout}</p>
              <Link
                href='/dashboard'
