...
export default function ProducerDetailPage() {
  ...
  const allApproved =
    documents.length > 0 && approvedCount === documents.length;
  ...
  const handleUndoApprove = async (docId: string) => {
    setActionLoading(docId);
    try {
      const res = await adminAPI.reviewDocument(producerId, docId, "pending");
      if (res.success) {
        await loadProducer();
      }
    } catch (err: any) {
      console.error("Undo approve error:", err);
    } finally {
      setActionLoading(null);
    }
  };
  ...
  if (!producer) {
    return (
      <div className="max-w-4xl mx-auto space-y-6">
        <button
          onClick={() => router.push("/ops/producers")}
          className={`flex items-center gap-2 text-sm text-gray-600 hover:text-gray-900 ${josefinRegular.className}`}
        >
          <ArrowLeft className="w-4 h-4" /> Back to Producers
        </button>
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          <p className={`text-gray-500 ${josefinRegular.className}`}>
            Producer not found.
          </p>
        </div>
      </div>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="flex items-start justify-between">
          <div>
            ...
            <div
              className={`flex flex-wrap items-center gap-x-4 gap-y-1 mt-2 text-sm text-gray-500 ${josefinRegular.className}`}
            >
              <span>{user.email}</span>
              {user.country && <span>{user.country}</span>}
              {user.discountCodeUsed && (
                <span>Discount code: {user.discountCodeUsed}</span>
              )}
          ...
          {isVerified ? (
            <span
              className={`inline-flex items-center gap-1.5 bg-green-50 border border-green-200 text-green-700 rounded-full px-4 py-1.5 text-sm ${josefinSemiBold.className}`}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
          <div>
            <p
              className={`text-xs text-gray-500 uppercase tracking-wider ${josefinRegular.className}`}
            >
              Documents
            </p>
            <p
              className={`text-lg text-gray-900 mt-0.5 ${josefinSemiBold.className}`}
            >
              {documents.length}
            </p>
          </div>
          <div>
            ...
            <p
              className={`text-lg text-green-600 mt-0.5 ${josefinSemiBold.className}`}
            >
              {approvedCount}
            </p>
          </div>
          <div>
            ...
            <p
              className={`text-lg text-red-600 mt-0.5 ${josefinSemiBold.className}`}
            >
              {rejectedCount}
            </p>
          </div>
          <div>
            ...
            <p
              className={`text-lg text-amber-600 mt-0.5 ${josefinSemiBold.className}`}
            >
              {pendingCount}
            </p>
          </div>
        ...
        {isVerified && assessment?.verifiedAt && (
          <div
            className={`mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500 ${josefinRegular.className}`}
          >
            Verified on {formatDate(assessment.verifiedAt)} by{" "}
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5">
        <div className="flex items-center justify-between">
          <p className={`text-sm text-gray-700 ${josefinSemiBold.className}`}>
            Products Listed
          </p>
          <span
            className={`text-sm text-gray-900 ${josefinSemiBold.className}`}
          >
            {products.length}
          </span>
        </div>
      ...
      <div className="space-y-3">
        <h2 className={`text-lg text-gray-900 ${titleFont.className}`}>
          Submitted Documents
        </h2>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3">
                        <p
                          className={`text-sm text-gray-900 truncate ${josefinSemiBold.className}`}
                        >
                          {doc.name}
                        </p>
                        <span
                          className={`inline-flex items-center gap-1 border rounded-full px-2.5 py-0.5 text-xs ${cfg.bg} ${cfg.color}`}
                        >
                      ...
                      <div
                        className={`flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-gray-500 ${josefinRegular.className}`}
                      >
                        {doc.category && (
                      ...
                      {doc.status === "rejected" && doc.rejectionReason && (
                        <div
                          className={`mt-2 flex items-start gap-2 bg-red-50 border border-red-100 rounded-lg px-3 py-2 ${josefinRegular.className}`}
                        >
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {!isVerified && doc.status === "approved" && (
                        <button
                          onClick={() => handleUndoApprove(doc._id)}
                          disabled={isLoading}
                          className={`inline-flex items-center gap-1.5 bg-gray-100 text-gray-800 rounded-lg px-3 py-1.5 text-xs hover:bg-gray-200 transition-colors disabled:opacity-60 ${josefinSemiBold.className}`}
                        >
                          Undo
                        </button>
                      )}
      ...
      {rejectModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className={`text-lg text-gray-900 ${titleFont.className}`}>
                Reject Document
              </h3>
              <button
                onClick={() => setRejectModal(null)}
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                ...
                disabled={
                  !rejectReason.trim() || actionLoading === rejectModal.docId
                }
              ...
              >
                {actionLoading === rejectModal.docId ? (
                  <Loader2 className="w-4 h-4 animate-spin" />
                ) : (
